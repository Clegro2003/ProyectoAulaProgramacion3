        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dgvPagos.SelectedRows.Count > 0)
                {
                    ...
                    string idmetodo = Convert.ToString(dgvPagos.SelectedRows[0].Cells["IDPago"].Value); 

                    DialogResult resultado = MessageBox.Show($"¿Está seguro de que desea eliminar este metodo {idmetodo}?", "Confirmación de Eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
